    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-text-muted text-sm">Manage your inventory and product listings.</p>
        </div>
        <Link
      ...
      <div className="bg-surface rounded-xl border border-border-main shadow-sm overflow-hidden">
          ...
          <thead>
            <tr className="bg-background border-b border-border-main text-text-muted uppercase text-[10px] font-bold tracking-widest">
              <th className="px-6 py-4 w-12">
                <input type="checkbox" className="rounded border-border-main text-[#95FF00] focus:ring-[#95FF00]" />
              </th>
            ...
            {loading ? (
              ...
              products.map((product) => (
                <tr key={product.id} className="hover:bg-zinc-50 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-text-muted">{product.vendorName || "No Vendor"}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <p className={`text-sm font-medium ${product.quantity === 0 ? 'text-red-500' : 'text-zinc-700'}`}>
        ...
        <div className="px-6 py-4 bg-background border-t border-border-main flex items-center justify-between">
          <p className="text-xs text-text-muted">Showing {products.length} products</p>
