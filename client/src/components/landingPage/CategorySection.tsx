...
interface CategorySectionProps {
  title: string;
  categorySlug: string;
  bgColor?: string; // no longer heavily used, but kept for compatibility
}
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Fetch categories to find the correct dynamic slug
        const catRes = await fetch(`${baseUrl}/categories`);
        const catData = await catRes.json();
        let slug = categorySlug;

        if (catData.success && catData.data) {
          // Filter matching categories and sort by product count to prefer populated categories
          const matchedCats = catData.data.filter(
            (c: any) =>
              c.slug === categorySlug ||
              c.slug.startsWith(`${categorySlug}-`) ||
              (c.title || c.name || "")
                .toLowerCase()
                .includes(title.toLowerCase()) ||
              title
                .toLowerCase()
                .includes((c.title || c.name || "").toLowerCase()),
          );
          if (matchedCats.length > 0) {
            matchedCats.sort(
              (a: any, b: any) => (b.productCount || 0) - (a.productCount || 0),
            );
            slug = matchedCats[0].slug;
          }
        }
        setResolvedSlug(slug);

        const res = await fetch(`${baseUrl}/products?category=${slug}&limit=4`);
        ...
        console.error(`Failed to fetch products for ${categorySlug}:`, err);
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="flex items-center justify-between mb-10">
          <div>
            ...
            <p className="text-text-muted mt-2 hidden sm:block">
              Explore the best products in {title.toLowerCase()}.
            </p>
          </div>
          <Link
            href={`/products?category=${resolvedSlug}`}
